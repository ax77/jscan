  ...
  public static CStorageKind combine_storage(List<Token> list) {
    ...
    for (Token tok : list) {
      ...
      if (tok.isIdent(typedef_ident)) {
        ...
        throw new AstParseException(tok.loc() + "error: expect storage-class-specifier, but was: " + tok.getValue());
      }
    }
    ...
    throw new AstParseException(lastLoc + "error:" + "unknown storage-class-specifier [" + ts.toString() + "]");
  }
  ...
  public static CTypeKind combine_typespec(List<Token> list) {
    ...
    for (Token tok : list) {
      ...
      if (tok.isIdent(void_ident)) {
        ...
        throw new AstParseException(tok.loc() + "error: expect type-specifier, but was: " + tok.getValue());
      }
    ...
    throw new AstParseException(lastLoc + "error:" + "unknown type-specifier [" + ts.toString() + "]");
  }

  private static void checkdup(int cnt, int max, String loc, String msg) {
    if (cnt >= max) {
      throw new AstParseException(loc + "error:" + "duplicate " + msg);
    }
  }

  private static void checkzero(int cnt, String loc, String msg) {
    if (cnt != 0) {
      throw new AstParseException(loc + "error:" + msg);
    }
  }
